...
# from start_migration import migrate_database
...
def starter_scraping_migration():
    ...
    print("Start add MongoDB")
    load_authors_from_file('authors.json')
    load_qoutes_from_file('quotes.json')
    print("End add MongoDB")

    # Проводимо міграцію
    results = subprocess.run(['python3', '-m', 'quotes.tuning.migration'], capture_output = True, text = True)
